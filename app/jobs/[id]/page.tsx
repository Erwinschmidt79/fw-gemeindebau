...
type Job = {
  ...
  gemeindebau: { adresse: string } | null;
};
...
export default function JobDetail() {
  ...
  useEffect(() => {
    let interval: any;
    async function tick() {
      const { data } = await sb.from("query_job").select("*, gemeindebau(adresse)").eq("id", jobId).single();
      ...
      if (data?.objectid) {
        ...
        const { data: t } = await q.order("stiege").order("tuernr_num", { ascending: true, nullsFirst: false });
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            ...
            <div className="text-sm text-slate-600">
              {job.gemeindebau?.adresse ?? "–"}{job.stiege ? " · Stiege " + job.stiege : ""}{job.tuernr ? " / " + job.tuernr : ""}
            </div>
          </div>
