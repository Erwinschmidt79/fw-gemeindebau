...
export default function MapClient() {
  ...
  // search is decoupled from rendering — only used to compute suggestions
  ...
  // -- FeatureCollections — built ONCE per data load (stable across typing) -
  ...
  // Stable key — only changes when data reloads, NOT on every keystroke.
  const dataKey = useMemo(() => String(bauten.length), [bauten.length])

  // -- Suggestions (cheap; doesn't touch the map layers) --------------------
  const suggestions = useMemo(() => {
    const q = search.trim().toLowerCase()
    if (q.length < 2) return [] as BauRow[]
    const out: BauRow[] = []
    for (const b of bauten) {
      if (
  ...
  return (
    <div className="relative h-[calc(100dvh-52px)] w-full">
      ...
      >
        ...
        {showPolygons && polygonsFC.features.length > 0 && (
          <GeoJSON
            key={`poly-${dataKey}`}
            data={polygonsFC}
            style={(feature: any) => {
        ...
        {showPins && pointsFC.features.length > 0 && (
          <GeoJSON
            key={`pts-${dataKey}`}
            data={pointsFC}
        ...
        <div className="flex-1 pointer-events-auto relative">
          <div className="bg-white rounded-xl shadow-lg flex items-center px-3 py-2">
            ...
            <input
              ...
              className="bg-transparent outline-none flex-1 text-sm text-slate-800 min-w-0"
            />
            {search && (
              <button
                onClick={() => { setSearch(''); setShowSuggestions(false) }}
                className="ml-1 text-slate-400 hover:text-slate-700 text-sm shrink-0"
          ...
          {showSuggestions && search.trim().length >= 2 && suggestions.length === 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-xl px-3 py-2 text-xs text-slate-500 z-[500]">
              Keine Treffer.
            </div>
          )}
        </div>
      ...
      {/* Layer toggles ------------------------------------------------------- */}
      <div className="absolute top-16 right-3 z-[400] bg-white/95 backdrop-blur rounded-lg shadow px-3 py-2 text-xs text-slate-700 space-y-1">
