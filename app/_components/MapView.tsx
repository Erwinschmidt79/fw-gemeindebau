...
export default function MapView() {
  ...
  // Search suggestions (max 8, only when typing)
  ...
  useEffect(() => {
    ...
    if (!showPins) return;
    // Cluster-Farbe basierend auf dominanter Kategorie der Kinder
    function clusterColor(markers: any[]): { bg: string; border: string } {
      const counts: Record<string, number> = {};
      for (const m of markers) {
        const cat = (m as any)._bau?.farb_kategorie ?? "grau";
        counts[cat] = (counts[cat] ?? 0) + 1;
      }
      const dominant = Object.entries(counts).sort((a, b) => b[1] - a[1])[0]?.[0] ?? "grau";
      const c: Record<string, { bg: string; border: string }> = {
        gruen:     { bg: "#16a34a", border: "#15803d" },
        hellgruen: { bg: "#86efac", border: "#22c55e" },
        gelb:      { bg: "#fde047", border: "#ca8a04" },
        orange:    { bg: "#fb923c", border: "#ea580c" },
        rot:       { bg: "#dc2626", border: "#991b1b" },
        grau:      { bg: "#cbd5e1", border: "#94a3b8" },
      };
      return c[dominant] ?? c.grau;
    }
    ...
    const cluster = L.markerClusterGroup({
      ...
      iconCreateFunction: (c: any) => {
        ...
        const col = clusterColor(children);
        const n = children.length;
        const size = n >= 100 ? 40 : n >= 10 ? 34 : 28;
        return L.divIcon({
          className: "",
          html: `<div style="width:${size}px;height:${size}px;border-radius:50%;background:${col.bg};border:2px solid ${col.border};color:white;display:flex;align-items:center;justify-content:center;font-size:${size < 34 ? 11 : 12}px;font-weight:700;box-shadow:0 2px 6px rgba(0,0,0,.25)">${n}</div>`,
          iconSize: [size, size],
  ...
  return (
    <div className="relative h-[calc(100vh-5rem)] overflow-hidden">
        ...
        <div className="flex-1 pointer-events-auto relative">
          <div className="bg-white rounded-xl shadow-lg flex items-center px-3 py-2">
            ...
            <input value={filter.search}
              onChange={e => { setFilter({...filter, search: e.target.value}); setShowSuggestions(true); }}
              onFocus={() => setShowSuggestions(true)}
              onBlur={() => setTimeout(() => setShowSuggestions(false), 150)}
              placeholder="Adresse oder Hofname …"
              className="bg-transparent outline-none flex-1 text-sm" />
            {filter.search && (
              <button onClick={() => { setFilter(f => ({...f, search: ""})); setShowSuggestions(false); }}
                className="ml-1 text-slate-400 hover:text-slate-700 text-sm">✕</button>
            )}
          ...
          {showSuggestions && suggestions.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-xl overflow-hidden z-[500]">
              {suggestions.map(b => (
                <button key={b.objectid}
                  onMouseDown={() => flyTo(b)}
      ...
      {/* Legende */}
      {showLegend && (showPolygons || showPins) && (
        <div className="absolute bottom-16 right-3 z-[400] bg-white/95 backdrop-blur rounded-xl shadow-lg p-3 text-[11px] max-w-[calc(100%-1.5rem)]">
          <div className="flex items-center justify-between mb-1.5">
            <div className="text-xs font-semibold text-slate-700">
              {showPolygons && showPins ? "Farb-Kategorien" : showPolygons ? "Gas-Anteil pro Bau" : "Heizungs-Status (Pins)"}
            </div>
          ...
          <div className="space-y-1">
            {(["gruen","hellgruen","gelb","orange","rot","grau"] as const).map(k => (
              <div key={k} className="flex items-center gap-2">
                {showPolygons && (
                  <span className="inline-block w-4 h-3 rounded-sm border flex-shrink-0"
                ...
                {showPins && (
                  <span className="inline-block w-3 h-3 rounded-full border-2 border-white flex-shrink-0"
                    style={{ background: FARB_MAP[k].fill, boxShadow: "0 1px 3px rgba(0,0,0,.3)" }} />
                )}
